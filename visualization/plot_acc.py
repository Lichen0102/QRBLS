...
class plot_tool():
    ...
    def download_acc(self,interval):
        ...
        acc_means = np.round(means_points * 100, 1)
        acc_stds = np.round(std_points * 100, 1)

        return acc_means[-1], acc_stds[-1]


    def plot_learning_curve(self, std_area, ax, axins, color, interval):
        self.read_pred_result()
        self.read_true_result()
        ...
        # plt.ylim(-0.2, 1.05)
        # plt.xlim(-10, 1050)
        if self.method == "IWDA_Multi":
            self.method = "IWDA"
        ax.plot(x_axis + self.x_shift, means_points, label="%s" % self.method, color=color, linewidth=self.linewidth)
        # plt.xlabel("Instances")
        # plt.ylabel("Accuracy")
        # plt.title('')

        if std_area:
            ax.fill_between(np.arange(x_axis.shape[0]) + self.x_shift, means_points - std_points,
                             means_points + std_points, interpolate=True, alpha=self.std_alpha, color=color)
        
        axins.plot(x_axis + self.x_shift, means_points, label="%s" % self.method, color=color, linewidth=self.linewidth)
        # plt.xlabel("Instances")
        # plt.ylabel("Accuracy")
        # plt.title('')

        if std_area:
            axins.fill_between(np.arange(x_axis.shape[0]) + self.x_shift, means_points - std_points,
                             means_points + std_points, interpolate=True, alpha=self.std_alpha, color=color)
