...
class IWDA_PL(object):
    def __init__(self, old_to_use=300, update_wm=500, whiten=False):
        self.weight_model_rw_1 = WeightModel(likelihood_model=MultivariateNormal(), reweighting_function=power_law_reweighting)
        self.lgbm_rw_1 = ErrorDriftModel(weight_model=self.weight_model_rw_1, ml_model=GaussianNB(), name="rw_oas_error",
                                    drift_detector=HDDM_W(), old_to_use=old_to_use, update_wm=update_wm,
                                    whiten=whiten)
        self.X_all = []
        self.y_all = []
    def fit(self, X_train, y_train):
        ...
        self.lgbm_rw_1.initial_fit(self.X_all, self.y_all)

    def predict_proba(self, x):
        x = pd.DataFrame(x, columns=['x{}'.format(i) for i in range(1, self.n_features+1)])
        x["batch"] = 0
        return self.lgbm_rw_1.predict_proba(x)
    def partial_fit(self, X, y):
        X = pd.DataFrame(X, columns=['x{}'.format(i) for i in range(1, self.n_features+1)])
        X["batch"] = 0
        y = pd.DataFrame(y, columns=["label"])
        
        error = (y != np.argmax(self.lgbm_rw_1.predict_proba(X)[0]))
        self.X_all = pd.concat((self.X_all, X), ignore_index=True)
        self.y_all = pd.concat((self.y_all, y), ignore_index=True)
        self.lgbm_rw_1.partial_fit(copy.deepcopy(self.X_all), copy.deepcopy(self.y_all), error)
    def predict(self, x):
